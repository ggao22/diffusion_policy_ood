...
def main(checkpoint, output_dir, device, screen_size):
    ...
    for i in range(tested_demos):
        ...
        for k in range(len(chunks)-1):
            ...
            centered_kp = centralize(kp_chunk, center_pos, center_ang) + np.array([512//2, 512//2])
            obs_in.append(centered_kp)
            
            centered_act = centralize(acts[chunks[k]:chunks[k+1]], center_pos, center_ang) + np.array([512//2, 512//2])
    ...
    #             if gen_random_kp:
    #                 kp_mean = generate_kp_traj(kp_mean,10)
    #             np_obs_dict = {
