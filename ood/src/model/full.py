...
class GmmFull(MixtureModel):
    ...
    def sample(self, n):
        mixture = Categorical(logits=self.logits.float())
        components = MultivariateNormal(self.mus, scale_tril=self.scale_tril.float())
        mixture_model = MixtureSameFamily(mixture, components)
        samples = mixture_model.sample(torch.Size([n]))
        return samples
